...
<body>
                ...
                <div class="card-body">
                    ...
                    <form action="/postEditBuku/{{ $data->id }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group mt-4">
                            ...
                            <input type="text" class="form-control border border-secondary" name="kodeBuku" required
                        ...
                        <div class="form-group mt-4">
                            ...
                            <input type="text" class="form-control border border-secondary" name="judulBuku" required
                        ...
                        <div class="form-group mt-4">
                            ...
                            <input type="text" class="form-control border border-secondary" name="penulis" required
                                value="{{ $data->penulis }}">
                            <span class="text-danger">
                                @error('penulis')
                                    {{ $message }}
                        ...
                        <div class="form-group mt-4">
                            ...
                            <input type="text" class="form-control border border-secondary" name="penerbit" required
                                value="{{ $data->penerbit }}">
                            <span class="text-danger">
                                @error('penerbit')
                                    {{ $message }}
                        ...
                        <!-- Kategori Buku Select -->
                        <div class="form-group mt-4">
                            <label class="text-secondary mb-2">Kategori Buku</label>
                            <select class="form-select" name="kategori">
                                @foreach ($kategori as $option)
                                    <option value="{{ $option }}"
                                        @if ($option == $data->kategori) selected @endif>{{ $option }}</option>
                                @endforeach
                            </select>
                        </div>

                        <!-- Deskripsi Buku Textarea -->
                        <div class="form-group mt-4">
                            <label class="text-secondary mb-2">Deskripsi Buku</label>
    ...
    <script>
        @if (Session::get('berhasil'))
            setTimeout(function() => {
                window.location.href = "{{ route('admin.buku') }}";
            }, 900);
        @endif
    </script>
